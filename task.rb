...
def q9
  ...
  # 以下に回答を記載
  names.each.with_index(1){ |value,index| puts "会員No.#{index} #{value}さん" }
...
def q11
  ...
  sports.flatten!.uniq!
  puts "ユーザーの趣味一覧"
  sports.each.with_index(1){ |value,index| puts "No#{index} #{value}" }
...
def q16
  ...
  # 以下に回答を記載
  users.each { |key|puts "私の名前は#{key[:name]}です。年齢は#{key[:age]}歳です。" }
